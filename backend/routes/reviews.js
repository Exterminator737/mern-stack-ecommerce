...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      // Determine verified purchase (paid order containing this product)
      ...
      const review = new Review({
        ...
        title,
        comment,
        photos: Array.isArray(photos) ? photos.filter(Boolean).slice(0, 5) : [],
        verifiedPurchase: Boolean(hasPaidOrder),
      });
...
router.put("/:id/helpful", auth, async (req, res) => {
  try {
    ...
    if (review.helpfulUsers.includes(req.user.id)) {
      return res
        .status(400)
        .json({ message: "You already marked this as helpful" });
    }
