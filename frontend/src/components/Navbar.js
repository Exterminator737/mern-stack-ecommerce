...
import { ShoppingBag, Menu, X, LogOut, User, Package, Heart, Settings, ChevronDown, Box, Search } from 'lucide-react';
        ...
        <div className="flex justify-between h-16 items-center gap-4">
          ...
          <div className="flex-shrink-0 flex items-center">
            <Link to="/" className="flex items-center gap-2 text-xl font-bold text-gray-900 hover:text-primary-600 transition-colors">
              ...
              <span className="hidden sm:block tracking-tight">Wholesale ZA</span>
            </Link>
          </div>
          ...
          <div className="hidden md:flex items-center space-x-6">
            ...
            <div className="flex items-center gap-1">
              {isAuthenticated && (
                <Link to="/wishlists" className="text-gray-600 hover:text-primary-600 transition-colors p-2" title="Wishlist">
                  <Heart className="h-6 w-6" />
                </Link>
              )}

              {/* Cart */}
              <Link to="/cart" className="relative p-2 text-gray-600 hover:text-primary-600 transition-colors" title="Cart">
            ...
            {isAuthenticated ? (
              ...
              <div className="flex items-center space-x-4">
                ...
                <Link to="/register" className="btn btn-primary shadow-sm shadow-primary-200">
                  Sign Up
                </Link>
              </div>
            )}
          ...
          <div className="md:hidden flex items-center gap-2">
            {isAuthenticated && (
              <Link to="/wishlists" className="p-2 text-gray-600 hover:text-primary-600">
                <Heart className="h-6 w-6" />
              </Link>
            )}
            ...
            {isAuthenticated ? (
              <>
                <div className="border-t border-gray-200 my-2 pt-2">
                  ...
                  <Link to="/profile" className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-50 flex items-center gap-2">
                    <User className="h-4 w-4" /> Profile
                  </Link>
                  <Link to="/orders" className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-50 flex items-center gap-2">
                    <Box className="h-4 w-4" /> Orders
                  </Link>
                  <Link to="/wishlists" className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-50 flex items-center gap-2">
                    <Heart className="h-4 w-4" /> Wishlists
                  </Link>
                  {isAdmin && (
                    <Link to="/admin" className="block px-3 py-2 rounded-md text-base font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-50 flex items-center gap-2">
                      <Settings className="h-4 w-4" /> Dashboard
                    </Link>
                  )}
              ...
              <>
                ...
                <Link to="/register" className="block px-3 py-2 rounded-md text-base font-medium text-primary-600 hover:bg-gray-50">Sign Up</Link>
              </>
            )}
          </div>
        </div>
